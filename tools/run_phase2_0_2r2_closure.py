...
def run_phase2_0_2r2_closure():
    ...
    # 2. 从当前 settings 和 Git 历史分别生成 model_config_hash
    current_model_config = build_canonical_model_config(settings)
    current_model_config_hash = compute_model_config_hash(current_model_config)

    reconstruct_ok, hist_hash_or_err, source_model_config = reconstruct_source_model_config_from_git(
        source_commit=model_evidence_source_commit,
        project_root=PROJECT_ROOT
    )
    if not reconstruct_ok:
        raise ValueError(f"Historical model config reconstruction failed: {hist_hash_or_err}")
    ...
    prev_meta = {
        ...
        "model_config_hash": hist_hash_or_err
    }
    ...
    model_config_hash_valid = bool(current_model_config_hash == hist_hash_or_err)
    ...
    gate_matrix = evaluate_all_gates(
        ...
        model_config_hash_valid=model_config_hash_valid,
    ...
    artifact_reuse_data = {
        ...
        "feature_schema_hash": feature_schema_hash,
        "model_config_hash": current_model_config_hash,
        "reconstructed_source_model_config_hash": hist_hash_or_err,
    ...
    # 9. 生成正式 Markdown 报告
    report_content = f"""# Phase 2.0.2r2 — Final Certification Closure Report
# A股模型研究认证真实性最终闭环与全量门禁实证报告

## 1. Git 溯源与血缘分层 (Git Provenance)

- **Run ID**: `{run_id}`
- **Model Evidence Source Commit**: `{model_evidence_source_commit}`
...
- **Model Config Hash**: `{current_model_config_hash}`
- **Reconstructed Source Model Config Hash**: `{hist_hash_or_err}`
...
| `MODEL_CONFIG_HASH_VALIDITY` | **{gate_matrix['MODEL_CONFIG_HASH_VALIDITY']}** | 从真实 settings 生成 canonical 模型配置 SHA256，且与历史 source commit 严格匹配 |
