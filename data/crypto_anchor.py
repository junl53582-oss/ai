...
2. ASYMMETRIC VERIFICATION: 生产环境优先使用成熟 cryptography 库 (RFC 8032 Ed25519)，验签仅依赖公钥。
3. KEY PURPOSE SEPARATION: 严格实施密钥用途隔离 (allowed_purposes)，禁止跨协议或跨组件混用密钥。
4. DOMAIN SEPARATORS: 强制引入域分隔符，彻底防御跨协议/跨上下文重放攻击。
5. KEY ROTATION & VALIDITY: 支持 not_before / not_after 与 REVOKED / EXPIRED 状态吊销机制。
6. ZERO HARDCODED SECRETS: 生产私钥仅由安全环境变量注入；缺失时自动 Fail-Closed。
"""
import os
import re
import json
...
# =========================================================================
# 1. 域分隔符常量定义 (Domain Separators - 防跨协议重放)
# =========================================================================
...
# =========================================================================
# 2. RFC 8032 Ed25519 纯 Python 算法 (Reference / Fallback)
# =========================================================================
...
# =========================================================================
# 4. 受信任操作员注册表 (Trusted Operator Registry)
# =========================================================================
...
def sign_with_environment_key(
    ...
    严格实施 Key Purpose 检查与 Domain Separator 前缀隔离。
    """
    if key_id not in TRUSTED_KEY_REGISTRY:
...
def verify_ed25519_signature(
    ...
    支持 Purpose 检查、Domain Separator 隔离与时间窗口/吊销检查。
    """
    errors = []
